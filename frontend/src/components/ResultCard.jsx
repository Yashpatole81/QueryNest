...
const ResultCard = ({ result, isSingle }) => {
  ...
  if (!success) {
    return (
      <div className={clsx(
        "flex flex-col gap-4", 
        !isSingle && "bg-white dark:bg-[#111111] border border-red-200 dark:border-red-900/50 rounded-2xl p-6 shadow-sm"
      )}>
        {!isSingle && (
  ...
  return (
    <div className={clsx(
      ...
      !isSingle && "bg-white dark:bg-[#111111] border border-slate-200 dark:border-zinc-800 rounded-2xl p-6 shadow-sm hover:shadow-md"
    )}>
      {!isSingle && (
